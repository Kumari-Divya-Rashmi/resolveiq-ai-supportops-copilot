...
if (parsed.success && parsed.data.NODE_ENV === "production") {
  ...
  if (missing.length) {
    throw new Error(`Invalid production environment configuration: ${missing.join(", ")} required`);
  }
}
...
export const env =
  parsed.success
    ? {
        ...parsed.data,
        USE_MEMORY_DB: parsed.data.USE_MEMORY_DB || (!parsed.data.MONGODB_URI && parsed.data.NODE_ENV !== "production"),
        AUTO_SEED: parsed.data.AUTO_SEED || (!parsed.data.MONGODB_URI && parsed.data.NODE_ENV !== "production"),
        JWT_SECRET:
